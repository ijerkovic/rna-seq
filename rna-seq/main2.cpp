...
int checkArguments(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    cout << "Usage: rna-seq <aligner> <genome.fa> <index_dir> <reads.fq> <result_file> <optional-gtf-file>" << endl;
    cout << endl;
...
void extractReadAlignmentDataFromSamFile(string samFilename) {
  ...
  while(getline(samFile, line)) {
    ...
    if(cols[3] == "0") {
      ...
      fprintf(result, "chr19 %d %d %s\n", atoi(cols[3].c_str()) - 1, atoi(cols[3].c_str()) + 100 - 1, cols[0].c_str());
    }
  }
...
void prepareDataForHistogramDrawing() {
  ...
  for (int i = 0; i < matched_reads.size(); i++) {
    ...
    for (int j = matched_reads[i].map_position; j < matched_reads[i].map_position + matched_reads[i].length; j++) {
      hist[j]++;  
    }  
  }
